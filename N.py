...
def pinta_face(face, color, luz_direcional=None, luz_intensidade=0, width=0): # Luz pontual
    if (luz_direcional != None and luz_intensidade > 0):
        #incidence_vector = list(map(operator.sub, face[0], luz_ponto))
        radiance = luz_intensidade * numpy.cos(angle_between(normal_vector(face), luz_direcional[:3]))#incidence_vector))
        #color = Color(round(color.r * radiance), round(color.g * radiance), round(color.b * radiance), color.a)
        print(radiance)
        if (radiance < 0.1):
            radiance = 0.1
        print(radiance)
        print(round(color.r * radiance))
